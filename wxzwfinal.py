...
class FastNovelCrawler:
    ...
    async def crawl_complete_chapter_async(self, session: ClientSession, chapter_url: str, chapter_num: int) -> Tuple[
        ...
        if pagination_links:
            print(f"   📄 发现 {len(pagination_links)} 个分页，并发爬取中...")

            # 创建分页爬取任务
            page_tasks = []
            for page_url in pagination_links:
                task = self.extract_page_content(session, page_url)
            ...
            for i, result in enumerate(page_results):
                if isinstance(result, Exception):
                    self.logger.warning(f"分页 {i + 1} 爬取失败: {result}")
                    continue
